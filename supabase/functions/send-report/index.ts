...
async function daily(): Promise<{
  ...
  const [sales, cash, pend, agents, allCash, prods] = await Promise.all([
    ...
    db(`products?select=id,name,weight_g`),
  ]);
  const pm: any = {};
  ...
  const rows: string[] = [];
  const shA: any[][] = [["COMMERCIALES DU " + D], [], ["Commerciale", "Validées", "Montant validé", "kg vendus", "À valider", "Montant à valider", "Détail à valider"]];
  for (const n of names) {
    ...
    const kg = vs.reduce((x, s) => x + kgOf(s.lines), 0);
    ...
    rows.push(
      `<tr><td><b>${esc(n)}</b>${active ? "" : ' <span style="font-size:10px;color:#999">(hors équipe)</span>'}</td>` +
        `<td>${vs.length ? `<span class="ok">${vs.length} validée${vs.length > 1 ? "s" : ""} · ${money(vt)}${kg > 0.005 ? ` · ${new Intl.NumberFormat("fr-FR", { minimumFractionDigits: 1, maximumFractionDigits: 1 }).format(Math.round(kg * 10) / 10)} kg` : ""}</span>` : "—"}</td>` +
    ...
    shA.push([
      ...
      Math.round(vs.reduce((x, s) => x + kgOf(s.lines), 0) * 100) / 100,
      ps.length,
