...
| **📊 Fichier DGI officiel généré par l'app (v35.59)** | Les boutons **📊 Fichier DGI — État 301 (officiel)** et **📊 Fichier DGI — Annexe TVA (officiel)** de l'écran Exports produisent **le vrai classeur DGI** (.xlsm) : logos DGI, formules officielles (barème ITS/RICF recalculés par le fichier lui-même, TVA, totaux), feuille TradXML et **macro « Génération du XML »** — le bouton orange officiel qui crée le fichier transmissible pour e-Impôts. Technique : chirurgie ZIP côté navigateur (CompressionStream natif, zéro bibliothèque) sur les modèles officiels embarqués (dgi-ITS.xlsm / dgi-tva.xlsm, purgés de toute donnée étrangère) ; les cellules à formule ne sont jamais touchées (numérotation automatique, barème) ; le modèle est mis en cache pour un usage hors ligne. Vérifiez la feuille SAISIE, complétez si besoin, cliquez le bouton orange → importez sur e-Impôts. |
| **📄 Feuilles de saisie DGI « à coller » (v35.57)** | Deux nouveaux boutons dans l'écran Exports : **📄 Saisie DGI — État 301 (à coller)** et **📄 Saisie DGI — Annexe TVA (à coller)**. Ils produisent des fichiers Excel **répliques conformes des feuilles officielles du classeur DGI** (feuille SAISIE : tête NCC/exercice/mois, en-têtes lignes 14-15, données dès la ligne 16 · feuille DETAIL_TVA_S : en-têtes lignes 17-18, données dès la ligne 20, dates en série Excel) avec une feuille **MODE D'EMPLOI**. Usage prévu par la notice officielle DGI : copier/coller depuis un autre fichier Excel exporté du système d'information — ouvrez le classeur DGI officiel, collez (collage spécial → valeurs), puis cliquez **le bouton officiel « Génération du XML »** : c'est la macro DGI elle-même qui produit le fichier transmissible pour e-Impôts. Complète les boutons XML directs (v35.56) : deux voies, même conformité. |
| **📋 XML e-Impôts : conformité stricte au générateur DGI (v35.56)** | Les fichiers XML e-Impôts (État 301, annexe TVA) sont désormais produits **exactement comme la macro officielle DGI** (macro « MiseEnFormeEDI » des fichiers edi-annexe, extraite et répliquée) : **un élément par ligne** (séparés par CRLF), **suppression de tous les accents** et remplacements de caractères identiques (é→e, & →ET, parenthèses→espaces, % →pourcentage, € →EUR, ° →O, œ →OE…), **MAJUSCULES** sur identité/emploi/désignation (État 301), ligne ignorée si n° CNPS absent (⚠ avertissement affiché), et **nom de fichier officiel** `NCC-EDI-<type>-aaaammjj-hhmm.xml` (ex. `1900000X-EDI-etat_301_mensuel-20260914-1530.xml`). Champs et ordre : 23 champs (État 301) et 15 champs (TVA) dans l'ordre exact du générateur. Import sur e-Impôts, étape 3 « Transmission des annexes détaillées par EDI ». |
| **📋 Fichiers EDI XML e-Impôts (v35.55)** | L'application **génère elle-même les fichiers XML officiels de la DGI** (syntaxe EDI XML e-Impôts — la DGI l'autorise explicitement dans la notice de ses générateurs) : **📋 XML e-Impôts — État 301 (EDI)** et **📋 XML e-Impôts — Annexe TVA (EDI)** dans l'écran Exports, à côté des annexes Excel (lisibles, pour contrôle). État 301 : 23 champs officiels par salarié dans l'ordre exact (n° CNPS, identité, code emploi, régime, sexe, nationalité, local/expatrié, situation de famille, enfants, parts, salaires, avantages, brut imposable, RICF, ITS brut/net, indemnités) alimentés directement par la paie clôturée. Annexe TVA : 15 champs officiels par opération ouvrant droit à réduction (achats locaux : date, fournisseur, NCC fournisseur, référence, nature, montant HT, TVA, redevable, prorata). Fichiers à importer sur e-Impôts, étape 3 « Transmission des annexes détaillées par EDI ». Nouvelles saisies : **NCC de la société** (Réglages → Fiscalité, obligatoire), **N° CNPS + données familiales + code emploi** sur la fiche employé, **NCC fournisseur** sur les achats. Refus clair si le NCC manque. Suite tedi (40 suites vertes). |
| **🪪 Matricules employés (v35.54)** | Le champ Matricule annonce « auto si vide » : c'est maintenant vrai — un matricule **EMP-001, EMP-002…** est attribué automatiquement à l'embauche quand on laisse le champ vide (sans collision avec les matricules existants, doublons toujours refusés si saisis). L'**annexe ITS DGI** affiche « — » pour un employé sans matricule (plus aucun identifiant technique), et les données de démonstration sont complètes (FKS-001 à FKS-005). |
| **⚙️ Modèle d'import/export Sage paramétrable (v35.53)** | Nouveau bouton **⚙ Format Sage (import/export)** (écran Exports) : un **modèle réglé exactement comme l'assistant d'import de Sage 100** — délimiteur de champ (`;` `,` tabulation `|`), identificateur de texte (guillemet/aucun), séparateur décimal (virgule/point), séparateur de milliers, nombre de décimales, format de date (JJ/MM/AAAA · JJMMAA · AAAA-MM-JJ · MM/JJ/AAAA), ligne d'en-tête incluse ou non, et **correspondance des colonnes** (Code journal, Date de pièce, Compte général, Libellé, Débit, Crédit — jusqu'à 12 positions, colonnes ignorées possibles, anti-doublons par échange). **Aperçu vivant** du fichier généré pendant le réglage. Le modèle pilote **l'export** `Import_Sage.txt` **ET l'import** des journaux (le sélecteur « Format du fichier » de l'import propose Détection automatique ou le modèle enregistré). Remise à zéro en un clic (défauts Sage). |
| **🔗 Pont Sage 100 dans les deux sens (v35.52)** | L'export Sage produit désormais TROIS fichiers : le classeur **xlsx** et le **csv** (pour contrôle), et surtout **Import_Sage.txt** au format des imports paramétrables Sage 100 — point-virgule, **sans ligne d'en-tête**, colonnes `JOURNAL;DATE;COMPTE;LIBELLÉ;DÉBIT;CRÉDIT`, dates JJ/MM/AAAA, montants à virgule décimale — à intégrer dans Sage par **Échanges > Importer des écritures** (détection automatique du format). Nouveau : **📥 Importer un journal Sage** (écran Exports) — chargez le fichier des écritures exporté depuis Sage (point-virgule ou tabulation, avec ou sans en-tête, dates JJ/MM/AAAA ou JJMMAA) : l'historique devient **consultable dans l'application** (carte « Historique Sage importé », exportable en Excel), **sans incidence sur la caisse ni l'exploitation** — donc sans double comptabilité. Idéal pour reprendre les mois précédents lors d'une mise en service. Nouvelle suite tsage (40 suites vertes). |
| **🔍 Ultra-review v35.51 — durcissements issus de l'audit** | Audit complet après la série v35.45→v35.50 : **anti-doublons à la création des comptes** (même e-mail ou même code PIN refusés — impossible d'avoir deux personnes sur le même compte), **code PIN gestionnaire solide exigé** (4 chiffres minimum, chiffres uniquement — y compris au changement dans les Réglages), colonne PIN masquée dans le tableau des comptes en mode en ligne (elle ne sert qu'en local), correction de 2 fautes de frappe dans les messages. Vérifié par ailleurs : aucune fonction appelée manquante (175 définies), tous les scénarios d'accès testés (39 suites vertes). |
| **🗝️ Le patron n'est jamais enfermé dehors (v35.50)** | Correctif urgent : la v35.48 (compte supprimé = accès révoqué) refusait aussi **l'e-mail du patron lui-même** s'il n'était pas dans la liste des comptes. Désormais, quand un e-mail inconnu tape ses identifiants, une **fenêtre de sauvetage** s'ouvre : « Vous êtes le gestionnaire ? Retrouvez l'accès avec votre **code PIN gestionnaire** ». Bon PIN → l'e-mail est inscrit comme **manager** et la session s'ouvre. Si le PIN est encore le **1234 par défaut**, un **nouveau PIN est exigé** sur place (le trou du PIN public est refermé au premier sauvetage) ; mauvais PIN → refus net (un ex-employé muni d'un login ne peut pas se promouvoir manager). Le changement de code PIN gestionnaire est désormais accessible **aussi en mode en ligne** (Réglages, sous le mode de connexion). Nouvelle suite trescue (39 suites vertes). |
| **👤 Créer les comptes SANS ouvrir Supabase (v35.49)** | À la demande du patron : en mode en ligne, la création d'un compte crée **automatiquement le login d'accès aux données** (e-mail + mot de passe) — **plus besoin d'aller sur Supabase**. Réglages → 👤 Comptes : saisissez Nom, Rôle, e-mail (+ code PIN local) et éventuellement le mot de passe initial (**généré automatiquement si vide**) ; l'appli fabrique le login et affiche les identifiants à communiquer (📋 Copier). Bouton **🔑** sur chaque ligne : recréer le login, ou **✉️ Réinitialiser** le mot de passe (la personne reçoit un e-mail pour en choisir un nouveau). Cas gérés avec message clair : login déjà existant, inscriptions désactivées, confirmation par e-mail requise (selon réglage du projet), panne réseau. **Optionnel (5 min, une fois)** : coller la fonction **user-admin** (fichier `supabase/functions/user-admin/index.ts` — Supabase → Edge Functions → Create function → coller → Deploy → Secret USERMGT_KEY = la clé saisie dans Réglages → 👤 Comptes) ; alors le login est **actif immédiatement** à la création et **supprimé** à la suppression du compte — plus aucune ligne SQL à copier. Nouvelle suite tuseradd (38 suites vertes). |
| **🔒 Supprimer un compte = révoquer l'accès (v35.48)** | Corrections suite au signalement du patron : la suppression d'un compte **coupe désormais réellement l'accès** — ① à la réouverture de l'appli, un compte absent de la liste est **refusé** (fini la promotion silencieuse en manager pour un e-mail inconnu, et fini le rôle restitué par le cache local) ; ② si l'appli est **déjà ouverte** sur le téléphone de l'ex-caissier, la vérification toutes les 30 secondes **déconnecte la session** (« 🔒 Votre compte a été supprimé — accès révoqué ») ; ③ à la connexion, un e-mail sans compte reçoit « Aucun compte pour cet e-mail » au lieu d'entrer. À la suppression, l'appli rappelle honnêtement que **l'identifiant Supabase (e-mail + mot de passe) reste actif** et fournit le SQL à coller (`delete from auth.users where email = '…'`) pour le retirer aussi. Le cache local ne fait plus autorité qu'en cas de travail **hors-ligne** (liste des comptes inaccessible). (37 suites vertes.) |
| **🏆 Grand livre : TOTAL GÉNÉRAL sur la dernière page (v35.47)** | À la demande du patron : la **dernière page** du grand livre (aperçu/PDF) se termine par un bloc encadré **« TOTAL GÉNÉRAL DU GRAND LIVRE »** — cumul de **tous** les employés sur **toute** la période : BRUT imposable, total des retenues (CNPS + CMU + ITS + autres + avances), net imposable, RICF, **NET À PAYER — TOTAL GÉNÉRAL** (vert, mis en valeur) et coût employeur total, avec le nombre d'employés et de bulletins. Sur les périodes longues (trimestre, année), un **détail par mois** (brut et net à payer) précède le total. Confirmation au passage : **les comptes à accès restreint se créent dans l'appli** (Réglages → 👤 Comptes), sans jamais ouvrir Supabase. (37 suites vertes.) |
| **📖 Grand livre : colonnes aérées (v35.46)** | Suite à votre retour : le grand livre **respire** — **3 employés par page** au lieu de 5 (le document peut faire 3 pages ou plus, la numérotation suit), **police agrandie** (12 px dans le tableau, noms 11,5 px, NET À PAYER 13 px), **marges intérieures doublées** (7×9 px par cellule, interligne 1,5), colonne des rubriques **élargie** (136 px — plus de libellés coupés), en-tête et pied de page plus lisibles. Les noms ne s'entremêlent plus, chaque colonne garde sa largeur. (37 suites vertes.) |
| **🏦 La Banque, mode de paiement partout (v35.45)** | « 🏦 Banque » devient un mode de paiement dans **tout le système** : **ventes** (formulaire et lien terrain), **achats de café vert** (à l'enregistrement et au paiement fournisseur), **encaissement des impayés** (déjà en place), **achats d'emballages**, **avances sur salaire**, **clôture de paie** (« Payé sur : Banque »), et **opérations bancaires libres** (nouveau bouton « + Écrire une opération » sur l'écran Banque : versement/retrait). Toute écriture banque reste dans le **journal banque isolé** : sans incidence sur la caisse espèces/MoMo ni sur l'exploitation (non imputable), jamais bloquée par un solde caisse. Badges et exports étiquetés « Banque ». Nouvelle suite tbanqpay (37 suites vertes). |
| **📗 Grand livre : mise en forme soignée (v35.44)** | L'aperçu/PDF du grand livre passe au niveau supérieur : **en-tête de document** (société + logo, titre, période, page X/Y, date de génération), rubriques **groupées en sections** — IDENTITÉ, GAINS, RETENUES, RÉCAPITULATIF —, lignes clés mises en valeur (**BRUT** et **coût employeur** sur fond crème, **NET À PAYER** en vert gras), colonne **TOTAL** sur fond distinct, zébrage des lignes, bordures fines de registre, chiffres alignés, pied de page société/Doregoservices. Colonnes équilibrées, noms + matricule en en-tête, 5 employés par page A4 (suite automatique). Suites tlivre/troles enrichies (36 suites vertes). |
| **🔒 Rôles : le caissier garde son rôle au rechargement + Banque/Achats/Impayés (v35.43)** | Correction d'une faille : après un **rechargement de page**, la session était restaurée **sans le rôle** — un caissier retrouvait l'accès complet. Désormais le rôle est rétabli depuis la liste des comptes (source de vérité), avec **cache local de secours** hors-ligne, et un email inconnu n'est plus promu manager silencieusement. Nouvelle répartition du rôle **caissier** : Accueil, Caisse, **Banque**, **Achats vert** et **Impayés** (barre du bas incluse) ; commercial et atelier inchangés ; le filtrage s'applique partout (onglets, ☰ Menu, barre du bas, boutons rapides, **accès direct par URL renvoyé à l'accueil**). Grand livre PDF : les **noms d'employés reviennent à la ligne proprement** (avec matricule) — fini les noms entremêlés. Nouvelle suite troles (36 suites vertes). |
| **📗 Grand livre PDF façon Sage + recherche avant génération (v35.42)** | L'aperçu « 🖨 Aperçu / Imprimer / PDF » du grand livre passe en **présentation transposée Sage Paie** : les **rubriques du salaire se lisent sur la longueur de la A4** (Poste, Zone, Matricule, Base, Transport, Logement, primes, Brut, CNPS, CMU, Imposable, RICF, ITS, retenues, avances, **NET À PAYER**, coût employeur) et **chaque employé occupe une colonne** sur la largeur, avec une colonne **TOTAL** par page ; au-delà de 5 employés par page, **la suite part automatiquement sur la page suivante** (en-têtes répétés, numérotation « suite page n »). La **barre de recherche apparaît aussi dans la vue de génération** de la paie (avant même de générer les bulletins) — plus un seul écran de la Paie sans recherche. Suite tlivre enrichie (35 suites vertes). |
| **💰 Prime de rendement AU NET + grand livre PDF (v35.41)** | Nouvelle rubrique « **au net** » : le montant saisi est ce que l'employé **touche exactement** — l'outil calcule lui-même le **brut** correspondant (ITS + CNPS incluses) qui s'ajoute au salaire de base ; le bulletin affiche « (au net : … F) ». Option **pré-remplissage automatique** : **kg vendus par la commerciale du mois × 25 F/kg** (correspondance par nom employée/commerciale) — **kg et montant restent modifiables** au moment de la paie. Le **grand livre de paie s'imprime et se télécharge en PDF** (aperçu « 🖨 Aperçu / Imprimer / PDF » en plus de l'Excel), pour toutes les périodicités (mensuel/trimestriel/annuel/personnalisé). Nouvelle suite tprime : gross-up exact à ±1 F, 2 primes simultanées, pré-remplissage kg×25 vérifié (35 suites vertes). |
| **👥 Paie : recherche générale, bulletins PDF groupés, grand livre périodique (v35.40)** | La **barre de recherche s'étend à toute la Paie** : Employés (v35.39) + **bulletins du mois** (nom, poste) + **avances** (employé, note). Nouveau bouton **« 📄 Tous les bulletins — PDF / impression »** : écran dédié affichant tous les bulletins du mois, **un par page A4**, avec « 🖨 Tout imprimer / Enregistrer en PDF » (le navigateur produit le PDF ; le bulletin individuel reste accessible via 👁). Le **livre de paie devient un grand livre périodique** façon Sage Paie : **Mensuel / Trimestriel (T1–T4) / Annuel / Personnalisé (n mois)** — même Excel détaillé avec totaux. Nouvelle suite tlivre (34 suites vertes). |
| **👥 Paie : barème RICF officiel + recherche employé + rubriques modifiables (v35.39)** | Correction du calcul des parts fiscales : l'app appliquait 5 500 F par demi-part **dès la première part** — le barème officiel (réforme ITS 2024) donne **0 F à 1 part**, puis **5 500 F/mois (66 000 F/an) par demi-part supplémentaire**, jusqu'à **5 parts maximum (44 000 F/mois · 528 000 F/an)**. Le champ employé devient une **liste déroulante du barème complet** (1,0 → 5,0 parts avec le RICF affiché), le bulletin affiche « N parts ». Nouvelle **barre de recherche des employés** (nom, poste, matricule) dans Paie → Employés. Les **rubriques de primes sont modifiables** (✎ Modifier : nom, calcul, valeur, fiscalité — s'applique aux prochaines paies, les bulletins générés restent inchangés). Nouvelle suite tparts (33 suites vertes). |
| **📞 ULTRAREVIEW : téléphone corrigé + tests auto-réparants (v35.38)** | Les liens « Appeler » (connexion, ☰ Menu, 🛟 Support) composaient un numéro **tronqué à 8 chiffres** (+225 07 17 57 95) — ils composent désormais le numéro complet **+225 07 17 57 95 56**. Robustesse des tests : `tests/harness.js` **régénère seul** `/tmp/qr.js` et `/tmp/app.js` depuis `index.html` (fini les faux échecs après une purge de /tmp), et le nouveau `tests/runall.sh` (présent dans le dépôt) **rend la main en échec** au moindre test rouge — un déploiement ne peut plus continuer avec des tests KO. Nettoyage du dépôt public : doublons obsolètes supprimés à la racine (`index.ts` = ancienne v2.2 de la fonction, `tbanque.js`, `texp.js`, `tstock.js` périmés) — la fonction à coller est **`supabase/functions/send-report/index.ts` (v2.4)**. |
| **📖 Support : guide d'utilisation complet (v35.37)** | Onglet **🛟 Support** : deux nouveaux boutons — **« 📖 Guide d'utilisation complet »** (page `guide.html` : les 19 onglets expliqués un par un, de l'Accueil au Support, avec étapes et pièges à éviter, + les 8 règles d'or) et **« 📈 L'exploitation expliquée »** (page `explique-exploitation.html`). Les deux pages sont mises **en pré-cache hors-ligne** dans le service worker. Au passage : suppression du doublon de la ligne version sur l'écran Support. |
| **💰 Stock de départ valorisé au prix déclaré (v35.36)** | Ajustements (Production → ⚖️) : nouveau champ **« Coût unitaire (FCFA) — stock de départ »** pour les produits (F/sachet) et le café vert/torréfié (F/kg). Tant qu'aucun achat n'est enregistré, c'est CE prix qui valorise les stocks — fini le stock à 0 F quand le CMP n'existe pas encore. Dès que des achats existent, le coût moyen réel reprend la main ; les unités initiales gardent leur prix déclaré en première couche. ⚠️ Une fois dans SQL Editor : `alter table public.adjustments add column if not exists unit_cost numeric default 0;` |
| **📊 Totaux par article + banque hors caisse du boss (v35.35)** | Dans « Produits — détail » et « Emballages — détail » : ligne **TOTAUX PÉRIODE** par article (production +X · ventes −Y · ajust. · net) + ligne **STOCK FINAL (toutes périodes)** (totaux complets + valeur). Correction de cohérence : les règlements par banque n'apparaissent plus dans la « Caisse du jour » du point du boss — une note 🏦 les récapitule à part (app + fonction v2.4 à recoller). |
| **📊 Point des stocks Excel détaillé par produit (v35.34)** | L'onglet « Mouvements produits » devient **« Produits — détail »** : pour CHAQUE produit, ses mouvements datés (production, ventes, ajustements) puis **STOCK FINAL expliqué** (productions − ventes + ajustements) et sa valeur. Même chose pour les emballages (**« Emballages — détail »** par article : entrées/sorties, stock final, coût moyen, valeur) et le **café vert** (bloc d'explication : achats − torréfaction + ajustements = stock final, avant le journal détaillé). |
| **💳 Crédits & dettes : paiements partiels + banque (v35.33)** | Encaisser un crédit client ou payer un fournisseur **en plusieurs fois** (montant libre, refus si dépasse le reste) — par espèces, MoMo ou **banque**. Les règlements bancaires épongent les créances/dettes **sans toucher la caisse ni l'exploitation** : ils sont suivis dans le nouvel écran **🏦 Banque** (menu) avec solde, origines et **export Excel dédié** (Journal_Banque) — distinct du journal de caisse et des 6 journaux Sage. |
| **🏷️ Emballages : jamais négatif + stock initial (v35.33)** | Production et sorties d'emballages refusées au-delà du stock (dispo affiché). Nouveau bouton **📥 Initial** par article : enregistre le stock réellement présent au début du suivi (comptage physique). |
| **🏆 Palmarès mensuel des commerciales + kg (v35.33)** | Écran 👤 Commerciaux : carte **Palmarès du mois — temps réel** (choix du mois, 👑 meilleure désignée, total F, **total kg**, part %) + export Excel mensuel. Le point quotidien du boss affiche aussi les **kg vendus par commerciale** (email + Excel) — base utile pour les primes de paie. |
| **📄 Bulletin de paie : une page + parts fiscales (v35.33)** | À l'impression : format A4, marges réduites, lignes compactées, anti-coupure — le bulletin tient sur **une seule page**. Le **nombre de parts fiscales** figure désormais dans le bloc identité. |
| **📎 Le cron envoie aussi logo + Excel (fonction v2.2 → v2.3)** | Les points envoyés automatiquement (cron 22h) incluent désormais le **logo et le nom de la société** + la **pièce jointe Excel** (3 feuilles : Ventes du jour, Commerciales, Caisse du jour), comme les envois faits depuis l'app. À activer en **recollant la fonction send-report v2.2** (Supabase → Edge Functions → send-report → remplacer le code → Deploy — secrets conservés). |
| **🔊 Plus jamais d'envoi silencieux (v35.31)** | À l'heure du point, si l'envoi ne part pas, l'app **dit pourquoi** : email du boss manquant, clé manquante, mode local, compte gestionnaire, ou l'erreur exacte (fonction non déployée, clé différente…). Réglages → 📧 affiche l'**état en direct** (mode / boss / clé / heure / version) avec un bouton **「 ▶️ Envoyer le point du jour maintenant 」** qui montre l'erreur réelle s'il y en a une. |
| **🕐 Heure du point à la minute près (v35.30)** | Réglages → 📧 → Heure d'envoi du point : écrivez librement **13h30, 9h15, 22h00, 13h, 9:15** — refus explicite si invalide, confirmation à l'enregistrement (et info si le point du jour est déjà parti). L'envoi est vérifié **toutes les 30 s** et dès le réglage — plus besoin de naviguer dans l'app. L'app n'envoie que si elle est ouverte ; l'envoi automatique app fermée reste le cron Supabase (minutes possibles aussi). |
| **🌑 Jours sans activité : le boss est prévenu quand même + heure paramétrable (v35.29)** | Le point quotidien part **tous les jours**, même néant : « 🌑 Journée sans activité — aucune vente, aucun mouvement de caisse, aucun envoi reçu », avec la liste des commerciales (🔴 aucun envoi reçu). Heure d'envoi réglable : **Réglages → 📧 → Heure d'envoi du point** (22 par défaut — changez pour tester puis remettez 22). ⚠️ Fonction **send-report v2** à recoller (Supabase → Edge Functions) si déployée avant le 10/09/2026 : l'ancienne version rejetait les appels du cron — le point automatique de 22h ne pouvait pas partir. |
| **🛡️ Jamais de stock négatif — produits finis (v35.28)** | Toute vente (directe, modifiée, ou validée depuis 📥 À valider) vérifie le stock disponible **produit par produit** : refus immédiat avec le dispo exact et la quantité demandée. Une saisie terrain refusée **reste en attente** jusqu'à l'enregistrement de la production (🏭) ou d'un ajustement. Stocks déjà négatifs : les régulariser par une production ou un ajustement (ex. +225 pour un stock à −225). |
| **👥 Point du boss détaillé par commerciale (v35.28)** | Le point quotidien liste **chaque** commerciale, tous les jours : ✅ ventes validées (nombre + montant) · ⏳ à valider (nombre + montant + détail des produits, ligne par ligne) · rejetées éventuelles · 🔴 « Aucun envoi reçu » pour celles qui n'ont rien envoyé · ligne Vente directe + total en attente. Nouvelle feuille « Commerciales » dans l'Excel du point. |
