...
4. **Production — flux strict** : 🌱 vert → 🔥 torréfaction → ⚙️ **Étape 1 Transformation** (machines : perte et rendement mesurés, quantités obtenues par type) → 📦 **Étape 2 Conditionnement** (ne consomme **que** les types issus des machines + emballages automatiquement — les produits doivent avoir leur type défini dans 🛍️ Produits). Transformation et conditionnement sont **réunis sur le même onglet** dans l'ordre du process. Tout est modifiable (✎) : transformations, conditionnements (emballages re-consommés au juste montant), et mouvements d'emballages (✎ dans le journal, écriture de caisse synchronisée).
