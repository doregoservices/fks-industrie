...
| **📎 Le cron envoie aussi logo + Excel (fonction v2.2)** | Les points envoyés automatiquement (cron 22h) incluent désormais le **logo et le nom de la société** + la **pièce jointe Excel** (3 feuilles : Ventes du jour, Commerciales, Caisse du jour), comme les envois faits depuis l'app. À activer en **recollant la fonction send-report v2.2** (Supabase → Edge Functions → send-report → remplacer le code → Deploy — secrets conservés). |
