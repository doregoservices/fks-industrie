...
await createSale({date:D,agent_name:'ANCIENNE',pay_mode:'cash',total:3000,lines:[{name:'PRODUIT LIBRE',qty:3,price:1000}]});
await createSale({date:D,agent_name:'Vente directe',pay_mode:'cash',total:1000,lines:[{name:'PRODUIT LIBRE',qty:1,price:1000}]});
const rep=await buildDailyReport(D);
const h=rep.html;
['ALICE','FATOU','SILENCE','ANCIENNE','hors équipe','à valider','Aucun envoi reçu','Vente directe','rejetée'].forEach(x=>{if(!h.includes(x))throw new Error('point du boss sans « '+x+' »');});
